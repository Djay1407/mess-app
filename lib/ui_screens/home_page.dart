...
import 'package:mess_app/widgets_utils.dart/sections.dart';
import 'package:mess_app/widgets_utils.dart/ui_features.dart';
import 'package:mess_app/widgets_utils.dart/drawer.dart';
import 'package:mess_app/style/theme.dart' as Theme;



const Color _kAppBackgroundColor = const Color(0xFFFFA726);   //BACKGROUND COLOR
...
class _AllSectionsLayout extends MultiChildLayoutDelegate {
  ...
  void performLayout(Size size) {
    final double columnCardX = size.width / 4.0;                      //CHANGE CARD PROPERTIES
    final double columnCardWidth = size.width - columnCardX;
    final double columnCardHeight = size.height / (cardCount+1);
    final double rowCardWidth = size.width;
    final Offset offset = translation.alongSize(size);
    double columnCardY = columnCardHeight/2;                                         //LEAVE SPACE FROM TOP
    double rowCardX = -(selectedIndex * rowCardWidth);

    // When tCollapsed > 0 the titles spread apart
    final double columnTitleX = size.width / 10.0;                  //HEADING POSITION
    ...
    const double paddedSectionIndicatorWidth = kSectionIndicatorWidth + 8.0; //DISTANCE BETWEEN THE INDICATORS
    final double rowIndicatorWidth = paddedSectionIndicatorWidth +
      (1.0 - tCollapsed) * (rowTitleWidth - paddedSectionIndicatorWidth);     //DISTANCE BETWEEN THE INDICATORS
    double rowIndicatorX = (size.width - rowIndicatorWidth) / 2.0 - selectedIndex * rowIndicatorWidth;    //POSTITION OF THE INDICATOR
...
@override
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
        ...
        key: _scaffoldKey,
        backgroundColor: Theme.Colors.loginGradientStart,   //BACKGROUND COLOR IN ROW FORMAT
        drawer: MessDrawer(),                      //NAVIGATION DRAWER
        body: Container(
...
Future<bool> _onWillPop() {
  ...
  Widget _buildBody(BuildContext context) {
    final MediaQueryData mediaQueryData = MediaQuery.of(context);
    final double statusBarHeight = mediaQueryData.padding.top;                  //SPACINGS IN COLUMN VIEW
    final double screenHeight = mediaQueryData.size.height;
    final double appBarMaxHeight = screenHeight - statusBarHeight;
